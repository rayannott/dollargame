...
def display_labels(G, sandbox, num_moves=None):
    ...
    for n in G.nodes():
        ...
        pos = G.nodes[n]['pos']
        screen.blit(text_node_vals, (pos[0]+int(NODERADIUS/1), pos[1]+int(NODERADIUS/2)))
...
def GameWindow(g, filename=None):
    ...
    while running_game:
        ...
        pygame.draw.rect(screen, (254, 151, 0) if not is_victory else GREEN, [0, 0, WIDTH, HEIGHT], 4)
        # play field
        pygame.draw.rect(screen, (0, 0, 255), [WIDTH*0.2, 4, WIDTH*0.8-4, HEIGHT-8], 2)
        pygame.display.update()
